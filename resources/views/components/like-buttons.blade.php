<div class="flex items-center mt-2">
    <form action="{{ route('like.store', $tweet) }}" method="post">
        @csrf
        <div class="flex items-center mr-5">
            <svg viewBox="0 0 20 20" class="w-3 mr-1 {{ $tweet->isLikedBy( auth()->user() ) ? 'text-blue-500' : 'text-gray-500' }}">
    ...
    <form action="{{ route('like.destroy', $tweet) }}" method="post">
        @csrf
        @method('delete')
        <div class="flex items-center mr-2">
            <svg viewBox="0 0 20 20" class="w-3 mr-1 {{ $tweet->isDislikedBy( auth()->user() ) ? 'text-blue-500' : 'text-gray-500' }}">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
